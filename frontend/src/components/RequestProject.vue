...
export default {
  ...
  async mounted () {
    ...
    let requestResponse = await Axios.get(`http://localhost:8091/get/all-termination-request/idUser?idUser=${this.idUser}`)
    ...
    for (let i = 0; i < requestArr; i++) {
      ...
      let requesterResponse = await Axios.get(`http://localhost:8090/get/user/id?id=${idRequester}`)
  ...
  methods: {
    ...
    async approveRequest (row) {
      ...
      let cardResponse = await Axios.get(`http://localhost:8091/get/project-card/projectName?projectName=${this.approveObj.projectName}&projectCardName=${this.approveObj.cardName}`)
